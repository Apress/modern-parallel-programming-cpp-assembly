...
void InitConvData1Dx2(std::array<CD_1Dx2, 3>& cd, const char* fn, size_t im_size)
{
    GetKernel1Dx2(cd[0], KERNEL_ID::LowPass1Dx2_9x9);
    GetKernel1Dx2(cd[1], KERNEL_ID::LowPass1Dx2_9x9);
    GetKernel1Dx2(cd[2], KERNEL_ID::LowPass1Dx2_9x9);
    ...
    {
        // Create test image
        size_t ks2 = cd[0].m_KernelSize / 2;
        size_t im_h = im_size + 15 + 2 * ks2;
        size_t im_w = im_size + 15 + 2 * ks2;
        ImageMatrix im_tmp(im_h, im_w, PixelType::Gray8);
    ...
    im_src2.FillBorder<uint8_t>(cd[2].m_KernelSize / 2, (uint8_t)0);
    cd[2].m_ImH = im_src2.GetHeight();
    cd[2].m_ImW = im_src2.GetWidth();
    cd[2].m_ImSrc = im_src2.ToVector<float>();
    cd[2].m_ImDes.resize(cd[2].m_ImSrc.size());
    std::fill(cd[2].m_ImDes.begin(), cd[2].m_ImDes.end(), 0.0f);
    cd[2].m_ImTmp.resize(cd[2].m_ImSrc.size());
    std::fill(cd[2].m_ImTmp.begin(), cd[2].m_ImTmp.end(), 0.0f);
}
