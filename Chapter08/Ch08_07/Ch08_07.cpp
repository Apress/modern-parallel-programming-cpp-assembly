...
int main()
{
    ...
    {
        std::cout << "Ch08_07 exception: " << ex.what() << '\n';
    }
...
static void MatrixVecMulF32(void)
{
    ...
    for (size_t i = 0; i < num_vec; i++)
    {
        ...
        std::cout << "vec_b3: ";
        std::cout << "  " << std::setw(w) << vec_b3[i].W << ' ';
        std::cout << "  " << std::setw(w) << vec_b3[i].X << ' ';
        std::cout << "  " << std::setw(w) << vec_b3[i].Y << ' ';
        std::cout << "  " << std::setw(w) << vec_b3[i].Z << '\n';

        if (!VecCompare(&vec_b1[i], &vec_b2[i]))
            throw std::runtime_error("Error - vector compare failed");
        if (!VecCompare(&vec_b1[i], &vec_b3[i]))
            throw std::runtime_error("Error - vector compare failed");
    }
}
