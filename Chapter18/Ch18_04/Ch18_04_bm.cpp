...
void MatrixMulF32_bm(void)
{
    ...
    for (size_t i = 0; i < num_it; i++)
    {
         ...
         bmtt.Start(i, 1);
         MatrixMulF32_Aavx512(c2_p, a_p, b_p, sizes);
         bmtt.Stop(i, 1);
    }

    std::string fn = bmtt.BuildCsvFilenameString("Ch18_04_MatrixMulF32_bm");
    ...
    if (!is_equal)
        throw std::runtime_error("Matrix compare failed - MatrixMulF32_bm(void)");
}
